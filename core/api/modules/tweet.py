...
class Tweet:
    ...
    def __init__(self, id, content, medium):
        self.id = id
        # Replace #Estación<Name> with only <Name>
        self.content = re.sub(
            r"#Estación([A-Za-z]{1,})", Tweet.replace_function, content)
    ...
    def gen_capture(self, url):
        ...
        if (ret):
            file_name = ''.join(random.choice(string.ascii_lowercase)
                                for i in range(10))
            cv2.imwrite(os.path.join(settings.PROJECT_ROOT,
                        f"../api/modules/images/temp/{file_name}.jpg"), frame)
                ...
                for element in match1:
                    ...
                    if alias and alias.station.line.id == line:
                        ...
                        if alias.station_id not in data[line.id]:
                            data[line.id].append(alias.station)
